...
class Site:
    ...
    def get_random_occupant(self, species: Species, rng: np.random.Generator) -> Occupant:
...
class Lattice:
    ...
    def __init__(self, size: tuple[int, int]):
        ...
        self.sites: list[Site] = [Site(f"{i}x{j}") for i in range(size[0]) for j in range(size[1])]
        i: int
        site: Site
